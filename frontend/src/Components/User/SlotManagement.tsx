          ...
          {selectedDate && (
            <div className="mb-6 w-full">
              ...
              {timings.length > 0 ? (
                  ...
                  {timings.map((timing, index) => {
                    ...
                    return (
                      <div key={index} className="mb-4 p-4 border rounded-lg">
                        ...
                        {generateHourBlocks(timing.startTime, timing.endTime, bookedTimes).map(
                          (block, idx) => (
                            <button
                              key={idx}
                              onClick={() => handleTimeSelection(block)}
                              className={`${
                                selectedTime === block ? "bg-blue-500 text-white" : "bg-blue-200"
                ...
                <div className="text-gray-600">No time slots available for this day</div>
              )}
            </div>
          )}
