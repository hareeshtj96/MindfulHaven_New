...
function Dashboard() {
  ...
  const { therapists, status, error, search_Result } = useSelector(
  ...
  const goToFamilyTherapy = () => {
    navigate("/familyTherapy");
  };

  const goToIndividualTherapy = () => {
    navigate("/individualTherapy");
  }

  const goToCoupleTherapy = () => {
    navigate("/coupleTherapy");
  }
    ...
    <div className="p-6">
        ...
        <div className="space-y-4">
          {hasSearched && therapists.length === 0 ? (
            <div className="text-center text-gray-700">
              <p>
                No therapists found based on your search criteria. Please try a
                different term.
              </p>
            </div>
          ) : (
            <>
              ...
              {Array.isArray(therapists) &&
                therapists.map((therapist) => (
                  <div
                    key={therapist._id}
                    className="flex bg-white rounded-lg shadow-md p-4 space-y-4 md:space-y-0"
                  >
      ...
      <section className="mb-16">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="relative" onClick={goToFamilyTherapy}>
            <img
              src={familyImg}
              alt="image1"
          ...
          <div className="relative" onClick={goToIndividualTherapy}>
            <img
              src={individualImg}
              alt="image2"
              className="w-full h-auto rounded-lg"
          ...
          <div className="relative" onClick={goToCoupleTherapy}>
            <img
              src={coupleImg}
              alt="image4"
              className="w-full h-auto rounded-lg"
      ...
      <section className="mb-8">
        ...
        <div className="mb-4 flex flex-col sm:flex-row justify-center items-center space-y-4 sm:space-y-0 sm:space-x-2">
          <input
            type="text"
            placeholder="Search anything related to Mental Health and Wellness"
            className="p-4 border border-gray-300 rounded-lg w-full sm:w-[500px]"
            value={searchTermGemini}
          ...
          {[
            {
              ...
              answer:
                "You can see your therapist up to 4 times per month depending on your subscription plan",
            },
            {
              question: "Does MindfulHaven provide family therapy?",
              answer:
                "Yes, MindfulHaven offers a range of therapies including family therapy",
            },
            {
              question: "What if I don't like the therapists?",
              answer:
                "You can choose a different therapist at any time through your dashboard",
            },
          ].map((faq, index) => (
            <li key={index} className="w-full">
              <button
                className="w-full text-left text-lg font-semibold focus:outline-none p-2 bg-gray-200 rounded-lg hover:bg-gray-300"
