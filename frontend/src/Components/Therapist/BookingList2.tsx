...
function BookingsList() {
    ...
    useEffect(() => {
        if (therapistId) {
            dispatch(fetchBookingAppointments({ therapistId, page: currentPage, limit: itemsPerPage, status: activeTab }))
                .then((result) => {
                    const data = unwrapResult(result);
                    setTotalPages(data.totalPagesBooking)
    ...
    return (
        <div className="flex justify-center items-center min-h-screen bg-gray-100 px-4">
            <div className="w-full max-w-7xl p-4 sm:p-6 bg-white rounded-lg shadow-md">
                <h2 className="text-xl sm:text-2xl font-bold mb-4 text-center">Appointments</h2>

                {/* Tabs */}
                <div className="flex flex-wrap space-x-2 sm:space-x-4 mb-4 sm:mb-6 justify-center">
                    <button
