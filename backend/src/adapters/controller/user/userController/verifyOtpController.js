    ...
    const verifyOtpController = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
        try {
            ...
            if (decoded.otp === otp) {
                ...
                try {
                    const result = yield userRepository.createUser(userData);
                    res.json({ status: true, message: httpStatusCode_1.ResponseMessages.OTP_VERIFIED_USER_REGISTERED_SUCCESSFULLY, data: result });
                }
                catch (dbError) {
                    res.status(httpStatusCode_1.HttpStatusCode.INTERNAL_SERVER_ERROR).json({ status: false, message: httpStatusCode_1.ResponseMessages.FAILED_TO_SAVE_USER_DATA });
                }
            }
            else {
